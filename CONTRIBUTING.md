...
- Browser code is TypeScript in `site/client/`, bundled by esbuild at build time. It must enhance, not create, the page: everything works without JavaScript except search, theme switching, folding and comments, and a bundle that fails to load leaves the page in its no-script form.
- giscus is the only third-party code the site loads. The build emits a mount point and nothing else; `site/client/comments.ts` injects the widget the first time the reader scrolls to it, so a page that is only read costs no foreign request.
